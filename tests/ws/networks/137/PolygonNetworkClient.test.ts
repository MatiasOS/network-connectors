...
const WS_URLS = getTestWsUrls("matic-mainnet", ["wss://polygon-bor-rpc.publicnode.com"]);

describe("PolygonClient (WebSocket) - Basic Methods [strong]", () => {
  it("should get chain ID over WebSocket", async () => {
    const client = new PolygonClient({ type: "fallback", rpcUrls: WS_URLS });

    const result = await client.chainId();

    assert.strictEqual(result.success, true, "Should succeed");
    assert.ok(isHexString(result.data!), "Chain ID should be hex string");
    assert.strictEqual(result.data, "0x89", "Should be Polygon mainnet");
