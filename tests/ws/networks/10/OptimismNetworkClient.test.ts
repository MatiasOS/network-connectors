...
const WS_URLS = getTestWsUrls("opt-mainnet", ["wss://optimism-rpc.publicnode.com"]);

describe("OptimismClient (WebSocket) - Basic Methods [strong]", () => {
  it("should get chain ID over WebSocket", async () => {
    const client = new OptimismClient({ type: "fallback", rpcUrls: WS_URLS });

    const result = await client.chainId();

    assert.strictEqual(result.success, true, "Should succeed");
    assert.ok(isHexString(result.data!), "Chain ID should be hex string");
    assert.strictEqual(result.data, "0xa", "Should be Optimism mainnet");
